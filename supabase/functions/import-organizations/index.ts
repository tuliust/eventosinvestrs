...
const key = (v: unknown) => clean(v).normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase().replace(/[^a-z0-9]+/g, " ").trim()
...
function nameScore(a: string, b: string) {
  ...
  const cleanA = matchKey(a)
  const cleanB = matchKey(b)
  if (!cleanA || !cleanB) return 0
  if (cleanA === cleanB) return 0.98
  let score = diceScore(cleanA, cleanB)
...
function findMatches(name: string, organizations: any[]) {
  return organizations
    .map((organization) => {
      const score = nameScore(name, organization.name)
      return {
        id: organization.id,
        name: organization.name,
        score: Math.round(score * 100) / 100,
        reason: key(name) === key(organization.name) ? "Mesmo nome" : "Nome semelhante",
      }
    })
...
function parse(buffer: ArrayBuffer) {
  ...
  const get = (row: unknown[], ...names: string[]) => {
    for (const name of names) {
      const index = header.get(key(name))
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    let resolutions: Record<string, { action: "merge" | "create"; organizationId?: string }> = {}
    ...
    const [{ data: existing }, { data: segments }, { data: subsegments }, { data: orgTypes }] = await Promise.all([
    ...
    for (const item of items) {
      const options = findMatches(item.name, organizations)
      if (options.length > 0) {
      ...
      prepared.push({ item, options, incoming })
      if (preview.length < 20) preview.push({ rowKey: item.rowKey, name: item.name, taxId: item.taxId, sector: item.sector, subsector: item.subsector, type: item.organizationType, status: options.length > 0 ? "Revisar" : "Nova" })
    }
    ...
    const unresolved = prepared.filter(({ item, options }) => options.length > 0 && !resolutions[item.rowKey])
    if (unresolved.length > 0) {
      return reply({ error: `Revise ${unresolved.length} possível(is) duplicidade(s) antes de importar.` }, 409)
    }
    ...
    for (const { item, options, incoming } of prepared) {
      const resolution = resolutions[item.rowKey]
      let found: any = null

      if (options.length > 0 && resolution?.action === "merge") {
        if (!resolution.organizationId || !options.some((option: any) => option.id === resolution.organizationId)) {
        ...
        if (!found) return reply({ error: `A organização de destino para “${item.name}” não foi encontrada.` }, 400)
      }
      ...
      if (found) {
        ...
        for (const [field, value] of Object.entries(incoming)) {
          ...
          if (!present) continue
          if (mode === "overwrite" || output[field] === null || output[field] === undefined || output[field] === "" || (Array.isArray(output[field]) && output[field].length === 0)) output[field] = value
        }
        rowsById.set(output.id, output)
        updated += 1
        continue
      }
      ...
      if (duplicateNewId) {
        const current = rowsById.get(duplicateNewId)
        ...
        rowsById.set(duplicateNewId, output)
        continue
      }
    ...
    for (let index = 0; index < rows.length; index += 200) {
      const { error } = await admin.from("organizations").upsert(rows.slice(index, index + 200), { onConflict: "id" })
      if (error) throw error
    }

    await admin.from("audit_logs").insert({
      actor_id: userData.user.id,
      action: "organizations.imported",
      ...
      metadata: {
        ...
        updated,
        total: items.length,
        reviewed_matches: matches.length,
